...
These experiments illustrate the performance differences considering the 
...
Consequently, both adaptation processes are taking into account for the 
resulting adaptation and the presented scenario conditions. Hence, the scenarios
represent three different situations which might require a user interface 
adaptation:
...
\begin{itemize}
  \item First, an introduction of the problem to be evaluated is described.
  
  \item Next, the scenario situation is presented. It is also summarized with
  a table indicating the main characteristics of the three main entities.
  
  \item Third, the adaptation process by Imhotep and AdaptUI is detailed.
...
AdaptUI, this scenario aim to solve a situation in which it impedes the user 
...
northernmost and coldest regions of Finland. As can be seen in see
...
feedback, on whether the interaction with the presented adaptation is good 
...
temperature (-10~ºC) might make difficult to John to use his hands and properly
...
way to indicate a disability would be if the user configures the Imhotep user
profile with a concrete disability. This would lead to a series of 
...
to use his fingers with the usual precision. Considering these premises,
any adaptive system would just ignore the current situation (including Imhotep).
...
any context characteristic lacks Imhotep of accuracy when adapting the user 
interface to the current situation. Besides, it is not able to model not only 
an activity, but the consequences of it (as AdaptUI, which takes into account 
that the user is performing an activity that, due to the context conditions,
cannot carry out properly).
...
Karen is a 60 years woman with several disabilities due to her ageing. She is
colour blind, which means she has problems distinguishing several colours. In
addition, she suffers from a light hearing loss, which is not severe. She is
driving to the down town using a \ac{gps} mobile application. For this scenario, the
most significant information to infer is the impossibility to use the hands and
the impossibility to distract her, as Karen is driving. Besides, the user model
should consider the capabilities cited in Table~\ref{tbl:scenario2}, as she is
colour blind. Therefore, with this information a first user model is 
...
disabilities, the user model configures her capabilities as it is shown in 
...
the corresponding views is increased, the interaction channel allows voice 
...
of the text would be added. Voice control would also be provided, and volume
levels would be increased by indicating a hearing disability.
...
somehow represent this through the variables user profile. 
...
is blind and suffers hearing loss. Consequently, the resulting user interface
...
channel and through a few modifications in the displayed views. As this 
...
On the contrary, to be able to represent a similar situation with Imhotep, the
user variables profile has to be modified.
...
Patrick is a 25 years old boy who lives in London. He lost his eyesight when he
...
device usually is not enough depending on the context situation. Mostly traffic
and street noise usually makes difficult to Patrick to hear the messages read by
the device.

Table~\ref{tbl:scenario3} shows the configuration of the first model for this
scenario. As it can be seen in this table, Patrick needs from some adaptation to
...
entails several obstacles during the interaction, as problems to hear the text
...
alternative channel, so the user would be able of interacting alternatively and 
...
Imhotep models user visual disabilities in its variables profile under the
...
Regarding the variables profile it is very difficult for Imhotep to make the 
...
regarding a concrete user disability. Here, context or activities are not as
...
side programmed adaptations different UI would be available. The main setback is
that Patrick has to indicate the concrete physiological problem. For example, he
would have to configure a figure detailing the dioptres he has. Then, the 
Imhotep server would act consequently. According to this, if the concrete 
figure has not been considered in the adaptation process, the resulting UI 
might not be adequate. Another issue is that, the network dependency would make 
the adaptation process too long. AdaptUI covers this problem as it runs 100 \% 
in the mobile device.

The main differences when dealing with the user, context and device models in
this scenario is shown in Table~\ref{tbl:adaptui_vs_imhotep_scenario3_user} and 
